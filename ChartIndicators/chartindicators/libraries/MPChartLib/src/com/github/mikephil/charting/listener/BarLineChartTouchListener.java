...
public class BarLineChartTouchListener extends ChartTouchListener<BarLineChartBase<? extends BarLineScatterCandleBubbleData<? extends BarLineScatterCandleBubbleDataSet<? extends Entry>>>> {
    ...
    private OnLongPressDrawHighlightHandler onLongPressDrawHighlightHandler = new OnLongPressDrawHighlightHandler();
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_DOWN:
                onLongPressDrawHighlightHandler.setInitialTouchCoordinates(event.getX());
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mTouchMode == DRAG) {
                    mChart.disableScroll();
                    onLongPressDrawHighlightHandler.setWasDragged(true);
                    performDrag(event);
                ...
                onLongPressDrawHighlightHandler.setFinalTouchCoordinates(event.getX());
                break;
...
/*        if (l != null) {
        ...
        if (mChart.isDoubleTapToZoomEnabled()) {
            ...
            if (mChart.isLogEnabled())
                Log.i("BarlineChartTouch", "Double-Tap, Zooming In, x: " + trans.x + ", y: "
                        + trans.y);
        } else if(mChart.isDoubleTapToFitScreenEnabled()) {
            mChart.fitScreen();
            if (mChart.isLogEnabled())
                Log.i("BarlineChartTouch", "Double-Tap, Fit Screen");
        }

        // for returning to initial variant uncomment lines 496-498 and comment 517-520
        if (l != null) {
            l.onChartDoubleTapped(e);
            return super.onDoubleTap(e);
        }
    ...
    public void onLongPress(MotionEvent e) {
        ...
        if(mChart.isHighlightPerLongClickEnable()) {
            if(onLongPressDrawHighlightHandler.shouldHighlight()) {
                Highlight h = mChart.getHighlightByTouchPoint(e.getX(), e.getY());
                performHighlight(h, e);
            }
            onLongPressDrawHighlightHandler.reset();
        }
...
//    }
    ...
    public void computeScroll() {
        ...
        if (Math.abs(mDecelerationVelocity.x) >= 40 || Math.abs(mDecelerationVelocity.y) >= 40)     // changed from 0.01 to 40 as this code keeps calculating scroll when user does not see it
            Utils.postInvalidateOnAnimation(mChart); // This causes computeScroll to fire, recommended for this by Google
        else {
            // Range might have changed, which means that Y-axis labels
            // could have changed in size, affecting Y-axis size.
    ...
    private class OnLongPressDrawHighlightHandler {

        private float initialTouchCoordinates = 0f;
        private float finalTouchCoordinates = 0f;
        private boolean wasDragged;

        public void setInitialTouchCoordinates(float initialTouchCoordinates) {
            this.initialTouchCoordinates = initialTouchCoordinates;
        }

        public void setFinalTouchCoordinates(float finalTouchCoordinates) {
            this.finalTouchCoordinates = finalTouchCoordinates;
        }
        ...
        public boolean shouldHighlight() {
            return !wasDragged || (wasDragged && Math.abs(finalTouchCoordinates - initialTouchCoordinates) < 30);
        }
    }
}
