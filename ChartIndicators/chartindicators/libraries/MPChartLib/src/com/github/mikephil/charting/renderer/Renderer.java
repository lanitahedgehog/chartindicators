...
public abstract class Renderer {
    ...
    protected int mMinDisplayedIndex = 0;

    /** the maximum value on the x-axis */
    protected int mMaxDisplayedIndex = 0;
    ...
    public void calcXBounds(BarLineScatterCandleBubbleDataProvider dataProvider, int xAxisModulus) {
        ...
        mMinDisplayedIndex = dataProvider.getLowestVisibleXIndex();
        mMaxDisplayedIndex = dataProvider.getHighestVisibleXIndex();
        
        int subLow = (mMinDisplayedIndex % xAxisModulus == 0) ? xAxisModulus : 0;
        
        mMinX = Math.max((mMinDisplayedIndex / xAxisModulus) * (xAxisModulus) - subLow, 0);
        mMaxX = Math.min((mMaxDisplayedIndex / xAxisModulus) * (xAxisModulus) + xAxisModulus, (int) dataProvider.getXChartMax());
    }
}
