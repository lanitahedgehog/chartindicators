...
//    protected boolean mDoubleTapToZoomEnabled = true;
    protected DoubleTapBehavior doubleTapBehavior = DoubleTapBehavior.NONE;
    ...
    public void calculateOffsets() {
        if (!mCustomViewPortEnabled) {
            ...
            mViewPortHandler.restrainViewPort(
                    /*Math.max(minOffset, offsetLeft)*/0, //remove default padding from the left side of the chart
                    /*Math.max(minOffset, offsetTop)*/0, //removes default padding from top side of the chart
...
//        mDoubleTapToZoomEnabled = enabled;
        if(enabled) {
            doubleTapBehavior = DoubleTapBehavior.ZOOM;
        }
        else {
            doubleTapBehavior = DoubleTapBehavior.NONE;
        }
    }
...
//        return mDoubleTapToZoomEnabled;
        return doubleTapBehavior == DoubleTapBehavior.ZOOM;
    }

    /**
     * Set this to true to enable fitScreen option by double-tap on the chart.
     * Default: disabled
     *
     * @param enabled
     */
    public void setDoubleTapFitScreenEnabled(boolean enabled) {
        if(enabled) {
            doubleTapBehavior = DoubleTapBehavior.FIT_SCREEN;
        }
        else {
            doubleTapBehavior = DoubleTapBehavior.NONE;
        }
    ...
    /**
     * Returns true if fitScreen option via double-tap is enabled false if not.
     *
     * @return
     */
    public boolean isDoubleTapToFitScreenEnabled() {
        return doubleTapBehavior == DoubleTapBehavior.FIT_SCREEN;
    }
    ...
    private enum DoubleTapBehavior {
        ZOOM, FIT_SCREEN, NONE
    }
}
