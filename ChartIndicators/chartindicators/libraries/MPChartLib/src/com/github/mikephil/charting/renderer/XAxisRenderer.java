...
public class XAxisRenderer extends AxisRenderer {
    ...
    public void renderAxisLabels(Canvas c) {
        ...
        if (mXAxis.getPosition() == XAxisPosition.TOP) {
            ...
            drawLabels(c, mViewPortHandler.contentTop() - yoffset,
                    new PointF(0.5f, 1.0f), mXAxis.isAxisShowDates());
            ...
            drawLabels(c, mViewPortHandler.contentBottom() + yoffset,
                    new PointF(0.5f, 0.0f), mXAxis.isAxisShowDates());
    ...
    protected void drawLabels(Canvas c, float pos, PointF anchor) {
        ...
        int minimum = mXAxis.mFixLabelsListeners == null ? mMinX : mXAxis.mFixLabelsListeners.fixMin(mMinX);
        int interval = mXAxis.mFixLabelsListeners == null ? mXAxis.mAxisLabelModulus : mXAxis.mFixLabelsListeners.fixModulus(mXAxis.mAxisLabelModulus);

        for ( int i = minimum; i <= mMaxX; i += interval ) {
    ...
    /**
     * Method is added for MTP project to display dynamically changing xAxis labels(which are Dates).
     * The DateFormat is changed depending on scaling.
     * @param c
     * @param pos
     * @param anchor
     * @param axisDhowDates preset xAxis field "axisShowDates" to let this method format dates
     */
    protected void drawLabels(Canvas c, float pos, PointF anchor, boolean axisDhowDates) {
        if(!axisDhowDates) {
            drawLabels(c, pos, anchor);
        } else {
            SimpleDateFormat dateFormat;

            long leftDateValue = Long.parseLong(mXAxis.getValues().get(mMinX));
            long rightDateValue = Long.parseLong(mXAxis.getValues().get(mMaxX));

            DateTime dayLeft = new DateTime(leftDateValue);
            DateTime dayRight = new DateTime(rightDateValue);
            int days = Days.daysBetween(dayLeft, dayRight).getDays();

            if (days >= 7) {
                dateFormat = new SimpleDateFormat("d MMM", Locale.getDefault());
            }
            else if (days >= 1) {
                dateFormat = new SimpleDateFormat("EEE HH:mm", Locale.getDefault());
            }
            else {
            ...
            for ( int i = minimum; i <= mMaxX; i += interval ) {
                ...
                if (mViewPortHandler.isInBoundsX(position[0])) {
                    ...
                    String labelRow = mXAxis.getValues().get(i);

                    long labelInLong = Long.parseLong(labelRow);
                    Date date = new Date(labelInLong);
    ...
    public void renderGridLines(Canvas c) {
        ...
        for (int i = minimum, gridLineOrder = 0; i <= mMaxX; i += interval ) {

            position[0] = i;

            //if(i + interval <= mMaxX) {
                nextGridLinePosition[0] = i + interval;
            ...
            gridLinePath.reset();
            if ( mViewPortHandler.isInBoundsX(position[0]) || mViewPortHandler.isInBoundsX(nextGridLinePosition[0]) ) gridLineOrder++;
