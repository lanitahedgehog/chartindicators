...
public class YAxisRenderer extends AxisRenderer {
    ...
    public void renderAxisLabels(Canvas c) {
        ...
        if (dependency == AxisDependency.LEFT) {
            ...
            if (labelPosition == YAxisLabelPosition.OUTSIDE_CHART) {
 //in this case we calculate the left edge of text label which is the chart's right coordinate (chart viewport without labels) + fixed offset. The whole chart viewport
 // (with labels) will not match the chart view's right border
 /*               mAxisLabelPaint.setTextAlign(Align.LEFT);
                xPos = mViewPortHandler.contentRight() + xoffset;*/

 //in this case we calculate the right edge of text label which is the right view's border. And the text will be placed on the left from this point.
 // So, the whole chart viewport (with labels) will match the chart view's right edge
                mAxisLabelPaint.setTextAlign(Align.RIGHT);
                xPos = mViewPortHandler.contentRight() + mViewPortHandler.offsetRight();
            } else {
                mAxisLabelPaint.setTextAlign(Align.RIGHT);
                xPos = mViewPortHandler.contentRight() - xoffset;
            }
    ...
    public void renderAnnotations(Canvas c) {
        ...
        for (int i = 0; i < annotations.size(); i++) {
            ...
            mAnnotationPaint.setStrokeWidth(0);
            ...
            if(a.isLabelDrawEnabled()) {
                ...
                mAnnotationPaint.setTextSize(a.getTextSize());

                float labelLineHeight = Utils.calcTextHeight(mAnnotationPaint, label);

                //now draw rectangle next to annotation line
                RectF labelBoxCoord = new RectF(mViewPortHandler.contentRight(),
                        pts[1] - labelLineHeight, mViewPortHandler.contentRight() + mViewPortHandler.offsetRight(), pts[1] + labelLineHeight);
                c.drawRoundRect(labelBoxCoord, 5, 5, mAnnotationPaint);

                float yOffset = Utils.calcTextHeight(mAnnotationPaint, "A") / 2f + mYAxis.getYOffset();

                float fixedXpos = mViewPortHandler.contentRight() + mViewPortHandler.offsetRight()/2;

                mAnnotationPaint.setColor(a.getTextColor());
                mAnnotationPaint.setTextAlign(Align.CENTER);
